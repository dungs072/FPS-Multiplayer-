...
public class HandlePickUp : NetworkBehaviour
{
    ...
    public void PickUpItem(string nameItem = "", bool isSelect = false)
    {
        ...
        {
            if (refer.WeaponManager.IsMaxShootWeapon)
            {
                if(isSelect)
                {
                    refer.WeaponManager.ChangeWeaponIndex(4);
                }
                ...
                pickupCoroutine = StartCoroutine(DoPickUpItemDelay(nameItem, isSelect));
            }
            else
            {
    ...
    public void PickUpAttachment(string nameItem,bool isNotNone)
    {
        ...
        if(refer.WeaponManager.CurrentWeapon is ShootWeaponBase)
        {
            (refer.WeaponManager.CurrentWeapon as ShootWeaponBase).
                                ChangeCurrentAnimatorIntoNewAnimator(scope?.AnimatorOverride);
        }
